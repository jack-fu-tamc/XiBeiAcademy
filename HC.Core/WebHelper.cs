...
namespace HC.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
            ...
            else
            {
                if (SslEnabled())
                {
                    ...
                    if (!String.IsNullOrEmpty(sharedSslUrl))
                    {
                         ...
                         * but we cannot reference EISMS.BusinessLogic.dll assembly.
        ...
        /// <summary>
        /// 如果被请求的资源是一个典型的资源，也不需要处理的cms引擎，返回true。
        /// </summary>
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new HCException("EISMS needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        private bool TryWriteGlobalAsax()
        {
            try
            {
                //When a new plugin is dropped in the Plugins folder and is installed into EISMS, 
        ...
        /// <summary>
        /// Gets or sets a value that indicates whether the client is being redirected to a 
        /// new location using POST
        /// </summary>
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContext.Items["HC.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["eisms.IsPOSTBeingDone"]);
            }
            set
            {
                _httpContext.Items["HC.IsPOSTBeingDone"] = value;
            }
        }
    }
}
