...
namespace HC.Core.Common.logHelper
{
    public static class fileRWhelper
    {
        ...
        public static void deleteFileWithWholePath(string file)
        {
            if (File.Exists(file))
                File.Delete(file);
        }
        ...
        *****************************************/
        ...
        public static string ReadFile(string Path)
        {
            ...
            {
                var curentDay = System.DateTime.Now.Day.ToString();
                StreamReader f2;
                var processDay =int.Parse(Path.Substring(Path.Length - 6).Substring(0, 2)).ToString();
                if (curentDay == processDay)
                {
                    #region 复制文件到~\Lognet\TempLog
                    var newDir = HttpRuntime.AppDomainAppPath.ToString() + "Lognet" + "\\" + "TempLog" + "\\" + Path.Substring(Path.Length - 6);
                    CopyFile(Path, newDir);
                    f2 = new StreamReader(newDir, System.Text.Encoding.UTF8);
                    #endregion
                }
                else
                {
                     f2 = new StreamReader(Path, System.Text.Encoding.UTF8);//gb2312
                }
